...
const AchievementBadge = ({ achievement }: { achievement: Achievement }) => {
  return (
    <div 
      className={`glass-card-hover p-4 ${achievement.completed ? 'border-primary/40' : ''}`}
    >
      ...
      <div className="relative h-2 bg-black/20 rounded-full overflow-hidden mb-2">
        <div 
          className={`absolute left-0 top-0 h-full ${achievement.completed ? 'bg-primary' : 'bg-primary/40'}`}
